...
function splitAddrs(raw: string | undefined): string[] {
  ...
  return raw
    ...
    .map((s) => s.trim())
    .filter(Boolean)
    .map((a) => asAddress("SYNC_ACCOUNTS entry", a));
}
...
async function main() {
  ...
  const accounts =
    splitAddrs(process.env.SYNC_ACCOUNTS) ||
    [];
  const fallback = deployment.deployer ? [asAddress("deployment.deployer", deployment.deployer)] : [];
  const syncAccounts = accounts.length > 0 ? accounts : fallback;
  if (syncAccounts.length === 0) {
    throw new Error("No accounts provided. Set SYNC_ACCOUNTS=0x...,0x... (at minimum include the provider wallet).");
  }

  const syncCall = staking.interface.encodeFunctionData("reinitializeV4SyncRewardDebt", [syncAccounts]);
  const upgradeCall = staking.interface.encodeFunctionData("upgradeToAndCall", [newImpl, syncCall]);
  ...
  const txs: ProposalTx[] = [
    {
      ...
      value: "0",
      data: upgradeCall,
      description: `DualPoolStaking.upgradeToAndCall(newImpl, reinitializeV4SyncRewardDebt(${syncAccounts.length} accounts))`
    }
  ];
  ...
  const proposal = {
    ...
    transactions: txs,
    notes: [
      "Execute this once to rebaseline rewardDebt for legacy stakers and stop harvestRewards from reverting on impossible backlogs.",
  ...
  const tag = nowTag();
  const jsonOut = path.join(outDir, `multisig-proposal-${network.name}-staking-rewarddebt-sync-${tag}.json`);
  const mdOut = path.join(outDir, `multisig-proposal-${network.name}-staking-rewarddebt-sync-${tag}.md`);

  fs.writeFileSync(jsonOut, JSON.stringify(proposal, null, 2) + "\n", "utf8");

  const mdLines: string[] = [];
  mdLines.push("# DualPoolStaking RewardDebt Sync Proposal");
  mdLines.push("");
  ...
  mdLines.push("");
  mdLines.push("## Sync Accounts");
  mdLines.push("");
  proposal.syncAccounts.forEach((a: string) => mdLines.push(`- \`${a}\``));
  mdLines.push("");
