 ...
 * @dev FIX: Added amount validation and balance check in burn function
 */
contract Myntis is 
    Initializable,
    ERC20Upgradeable, 
    ...
    UUPSUpgradeable 
{
     ...
     * @dev FIX: Cap check correctly uses amount (includes fees)
     */
    ...
    function updateCap(uint256 newCap) external onlyRole(ADMIN_ROLE) {
        ...
        require(newCap >= totalSupply(), "Cap less than current supply");
        uint256 oldCap = _cap;
        _cap = newCap;
