 ...
 * @dev All security fixes from audit applied.
 */
contract MyntisOFT is ERC20, AccessControl, Pausable, ReentrancyGuard {
     ...
     * @dev FIX: Added input validation for zero address and zero amount
     */
    function quoteBridge(
        ...
        require(to != address(0), "MyntisOFT: zero recipient");
     ...
     * @dev FIX: Added null checks and reentrancy protection
     */
    function bridge(
        uint32 dstEid,
        address to,
        ...
        require(amount > 0, "MyntisOFT: zero amount");
        require(to != address(0), "MyntisOFT: zero recipient");
        require(refundAddress != address(0), "MyntisOFT: zero refund");
     ...
     * @dev FIX: Added reentrancy protection
     */
    function lzReceive(
    ...
    /**
     * @notice Set peer contract address for a given endpoint ID.
     * @dev FIX: Added validation to prevent setting zero peer
     */
    function setPeer(uint32 eid, bytes32 peer) external onlyRole(ADMIN_ROLE) {
        require(peer != bytes32(0), "MyntisOFT: cannot set zero peer");
        peers[eid] = peer;
     ...
     * @dev FIX: Added validation to prevent setting cap below current supply
     */
    function updateCap(uint256 newCap) external onlyRole(ADMIN_ROLE) {
        require(newCap <= _maxSupply, "MyntisOFT: cap > max");
        require(newCap >= totalSupply(), "MyntisOFT: cap < current supply");
     ...
     * @dev FIX: Added zero amount validation
     */
    ...
    /**
     * @notice Internal mint function with fee handling.
     * @dev FIX: Added amount validation. Cap check correctly uses amount (includes fees).
     *      The amount parameter represents the total supply increase (netAmount + feeAmount).
     */
    function _mintWithFee(address to, uint256 amount) internal {
        require(to != address(0), "MyntisOFT: mint to zero");
        require(amount > 0, "MyntisOFT: zero amount");
        // Cap check: amount includes fees, so totalSupply() + amount is correct
    ...
    /**
     * @notice Internal burn function with fee handling.
     * @dev FIX: Added balance check and zero amount validation
     */
    function _burnWithFee(address from, uint256 amount) internal {
        require(amount > 0, "MyntisOFT: zero amount");
        require(balanceOf(from) >= amount, "MyntisOFT: insufficient balance");
