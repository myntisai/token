...
describe("Comprehensive Myntis Ecosystem Testing", function () {
  ...
  beforeEach(async function () {
    ...
    stakingContract = await StakingContract.deploy(myntisToken.target, admin.address);
    ...
    emissionContract = await EmissionContract.deploy(myntisToken.target, stakingContract.target, admin.address);
    ...
    await myntisToken.connect(admin).grantRole(MINTER_ROLE, emissionContract.target);
    ...
    if (emissionContract.setStakingContract) {
      await emissionContract.connect(admin).setStakingContract(stakingContract.target);
    }

    // In the StakingContract, set the EmissionContract.
    await stakingContract.connect(admin).setEmissionContract(emissionContract.target);

    // Deploy the MerkleDistributor.
    const MerkleDistributor = await ethers.getContractFactory("MerkleDistributor");
    merkleDistributor = await MerkleDistributor.deploy(myntisToken.target, admin.address);
    await merkleDistributor.waitForDeployment();

    // Set the staking contract inside the MerkleDistributor.
    await merkleDistributor.connect(admin).setStakingContract(stakingContract.target);

    // Tell the StakingContract which MerkleDistributor to use.
    await stakingContract.connect(admin).setMerkleDistributor(merkleDistributor.target);

    // Deploy the Bridge contract (MyntisBridgeL1) using the MyntisToken as the token.
    const Bridge = await ethers.getContractFactory("MyntisBridgeL1");
    bridge = await Bridge.deploy(myntisToken.target, admin.address);
  ...
  describe("Staking & Unstaking by Providers", function () {
    it("should allow providers to register, increase stake, and withdraw stake", async function () {
      ...
      await myntisToken.connect(provider1).approve(stakingContract.target, ethers.parseEther("5000"));
      await myntisToken.connect(provider2).approve(stakingContract.target, ethers.parseEther("5000"));
      await myntisToken.connect(provider3).approve(stakingContract.target, ethers.parseEther("5000"));
      ...
      await myntisToken.connect(provider1).approve(stakingContract.target, ethers.parseEther("1000"));
  ...
  describe("Bridge Functionality", function () {
    it("should deposit tokens into the bridge and allow unlock by the operator", async function () {
      ...
      await myntisToken.connect(provider1).approve(bridge.target, depositAmount);
      const depositTx = await bridge.connect(provider1).deposit(depositAmount);
      await depositTx.wait();

      // Confirm that the bridge holds the tokens.
      const bridgeBalance = await myntisToken.balanceOf(bridge.target);
